...
MAX_IMAGES = 30  # use first N images for experiment
...
OUTPUT_DIR = os.path.join(BASE_DIR, "working", "threshold_experiment")
os.makedirs(OUTPUT_DIR, exist_ok=True)

CSV_PATH = os.path.join(OUTPUT_DIR, f"{SPLIT}_threshold_experiment.csv")

# Thresholds to test
THERMAL_PERCENTILES = [95, 97, 98, 99]
...
def build_candidate_mask(thermal, height_channel, thermal_percentile):
    ...
    # thermal threshold only within building-like area
...
for thermal_percentile in THERMAL_PERCENTILES:
    ...
    for img_info in images:
        ...
        candidate_mask, t_thr = build_candidate_mask(thermal, height_channel, thermal_percentile)
    ...
    results.append({
        "thermal_percentile": thermal_percentile,
...
for row in results:
    print(
        f"Threshold {row['thermal_percentile']}% | "
